 ...
 * etc.
 *
 * - The key codes are not very good (modifiers such as CTRL and META are
 * simply ignored).
 *
...
static void process_event(WINDOW *w, XEvent *ev)
{
  ...
  case ButtonPress:
	...
	w->last_click_sec = tv.tv_sec;
      ...
      w->mouse_x = bev->x;
      w->mouse_y = bev->y;
      w->mouse_b = bev->button & (MOUSE_1 | MOUSE_2 | MOUSE_3);
      if (SEND_MESSAGE(msg, w->dialog, 0) & D_CLOSE) {
	main_dlg->flags |= D_CLOSE;
	win_hidewindow(main_dlg->win);
      }
    }
...
int win_read_mouse(WINDOW *win, int *ret_x, int *ret_y)
{
  ...
  if ((! win->pointer_grabbed)
      && (x < 0 || y < 0 || x >= win->w || y >= win->h
	  || (! win->selected && ! win->is_child))) {
    /* The mouse if out of the window; let's return
     * the last known position and button status.
     */
    if (ret_x != NULL)
      *ret_x = win->mouse_x;
    if (ret_y != NULL)
  ...
  win->mouse_x = x;
  win->mouse_y = y;
  win->mouse_b = mask & (MOUSE_1 | MOUSE_2 | MOUSE_3);
